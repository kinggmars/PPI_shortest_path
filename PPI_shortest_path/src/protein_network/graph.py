...
class Graph:
    ...
    def floyd_warshall_export(self, filename):
        dist, path_matrix, id_to_node = self.floyd_warshall()
        n = len(dist)
            ...
            with open(filename, 'w') as f:
                ...
                for start_node in self.adj:
                    try:
                        ...
                        paths = self.dijkstra_shortest_paths(start_node)
                    except ValueError:
                        continue
    ...
    def bellmanford_export_all_paths(self, filename):
        with open(filename, 'w', encoding='utf-8') as f:
            for node in self.adj:
                results = self.bellmanford_shortest_paths(node)
                try:
                    f.write("node1 node2 total_weight path\n")
                    for line in results:
                        f.write(f"{line}\n")
